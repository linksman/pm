...
export const KanbanBoard = ({ onLogout, user }: { onLogout: () => void; user?: User }) => {
  ...
  const updateBoard = (updater: (prevBoard: BoardData) => BoardData) => {
    setBoard((prev) => {
  ...
  const sendAiMessage = async (question: string) => {
    ...
    try {
      const response = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          question,
          history: nextHistory.map(({ role, text }) => ({ role, content: text })),
        }),
      });
      ...
      setChatMessages((prev) => [
        ...
        {
          role: "assistant",
          text: "Sorry, the AI could not respond right now.",
        },
      ]);
    } finally {
      setAiLoading(false);
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-slate-50 px-6 py-24 text-slate-900">
        <div className="mx-auto w-full max-w-xl rounded-3xl border border-slate-200 bg-white p-10 shadow-lg text-center">
